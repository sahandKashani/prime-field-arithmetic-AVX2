...
void initialize_constants() {
    ...
    gmp_int_t a_gmp;
    gmp_int_t b_gmp;
    gmp_int_t points_x_gmp[NUM_POINTS / NUM_ENTRIES_IN_LIMB];
    gmp_int_t points_y_gmp[NUM_POINTS / NUM_ENTRIES_IN_LIMB];
    ...
    for (unsigned int i = 0; i < (NUM_POINTS / NUM_ENTRIES_IN_LIMB); i++) {
        gmp_int_set_strings(points_x_gmp[i], points_x_hex_glo + (i * NUM_ENTRIES_IN_LIMB), 16);
        gmp_int_set_strings(points_y_gmp[i], points_y_hex_glo + (i * NUM_ENTRIES_IN_LIMB), 16);
    }
    ...
    convert_gmp_to_num(a_glo, a_gmp, NUM_LIMBS);
    convert_gmp_to_num(b_glo, b_gmp, NUM_LIMBS);
    for (unsigned int i = 0; i < (NUM_POINTS / NUM_ENTRIES_IN_LIMB); i++) {
        convert_gmp_to_num(points_x_glo[i], points_x_gmp[i], NUM_LIMBS);
        convert_gmp_to_num(points_y_glo[i], points_y_gmp[i], NUM_LIMBS);
    }
