...
bool is_equal_num_gmp(uint64_t const *num, mpz_t const num_gmp, unsigned int const num_limbs) {
	return cmp_num_gmp(num, num_gmp, num_limbs) == 0;
}

int cmp_num_gmp(uint64_t const *num, mpz_t const num_gmp, unsigned int const num_limbs) {
	mpz_t tmp;
	mpz_init(tmp);
